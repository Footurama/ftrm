...
describe('subscribe', () => {
	...
	test('unsubscribe', () => {
		...
		const ipc = new IPC(bus);
		const addr = 'abc';
		const cnt = bus.on.mock.calls.length;
		ipc.subscribe(addr);
		expect(bus.on.mock.calls.length).toBe(cnt + 1);
		ipc.subscribe(addr);
		expect(bus.on.mock.calls.length).toBe(cnt + 1);
		ipc.unsubscribe(addr);
		expect(bus.removeListener.mock.calls.length).toBe(0);
		ipc.unsubscribe(addr);
		expect(bus.removeListener.mock.calls.length).toBe(1);
		expect(bus.removeListener.mock.calls[0][0]).toBe(bus.on.mock.calls[cnt][0]);
		expect(bus.removeListener.mock.calls[0][1]).toBe(bus.on.mock.calls[cnt][1]);
	});
});
