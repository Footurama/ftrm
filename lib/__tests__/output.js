...
test(`Retransmit last value`, () => {
	...
	o.value = value;
	expect(onPipe.mock.calls.length).toBe(1);
	expect(onPipe.mock.calls[0][0]).toBe(0);
	expect(onPipe.mock.calls[0][1]).toBe(value);
	jest.advanceTimersByTime(retransmit);
	expect(onPipe.mock.calls.length).toBe(2);
	expect(onPipe.mock.calls[1][0]).toBe(retransmit);
	expect(onPipe.mock.calls[1][1]).toBe(value);
});
