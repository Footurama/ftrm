...
test(`expire values`, () => {
	...
	const name = 'test';
	const TIMEOUT = 1000;
	const i = new Input({name, pipe: 'pipe', expire: TIMEOUT}, bus, log);
	...
	expect(log.warn.mock.calls[0][0].message).toEqual(`Input ${name} expired`);
...
test(`set log level for expiration`, () => {
	...
	new Input({pipe: 'pipe', expire: TIMEOUT, logLevelExpiration: null}, bus, log);
	const TS = 1234567890;
	Date.now = jest.fn(() => TS);
	bus.on.mock.calls[0][1](TS, 42);
...
test(`Set default value on expiration`, () => {
	...
	const bus = { on: jest.fn() };
	const expire = 1000;
	const i = new Input({pipe: 'pipe', expire, default: defaultValue, logLevelExpiration: null}, bus);
