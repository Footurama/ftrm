class Output {
	constructor (opts, bus, log, componentOpts) {
		this._bus = bus;
		this._log = log;
		this._source = {
			componentId: componentOpts.id,
			componentName: componentOpts.name
		};
	...
	set (value, timestamp) {
		...
		const q = (this.pipe) ? this._bus.emit(this.pipe, this.timestamp, this._value, this._source) : Promise.resolve(0);
