class Output {
	...
	set (value, timestamp) {
		...
		let q;
		if (this.pipe) {
			q = this._bus.emit(this.pipe, this.timestamp, this._value);
			q.then((cnt) => {
				if (cnt <= 0) this._log.warn(new Error(`Pipe ${this.pipe} had no listeners`), 'baaf4847f8857ac34dc75b51d44696da');
			});
		} else {
			q = Promise.resolve(0);
		}
