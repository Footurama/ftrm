...
class Input extends EventEmitter {
	constructor (opts, bus, log) {
		...
		async function eventListener (timestamp, value, source = {}) {
			...
			if (self.checkpoint) {
				try {
					...
					if (self.logLevelCheckpoint) {
						log[self.logLevelCheckpoint](err, '661c3f3b934be8c722405399fb41e2e6');
					}
					return;
				}
			}
			...
			if (self.expireError) {
				self.expireError.resolve();
				delete self.expireError;
			}
			if (opts.expire) {
				...
				self.expireTimeout = setTimeout(() => {
					...
					if (self.logLevelExpiration) {
						self.expireError = new StatefulError(`Value from pipe ${self.pipe} expired`);
